...
;
	...
	for (; y > 1; y--) { // for loop that will subtract 1 from y until y is 2 
		if (x%y == 0){ // checks if x divided by y is 0
			int variable = y;
			JOptionPane.showMessageDialog(null, x +" is not prime. It is divisible by "+ variable);// if so, prints "number is not prime"
			flag = 1;//changes flag
			break;//breaks loop to prevent more not prime messages
		}
		}
